...
/**
 * Reordena hijos del header: logo | menú | iconos (desktop; móvil agrupa iconos+menú vía JS).
 *
 * @param array<int,array<string,mixed>> $elements Hijos del contenedor shell.
 * @return array<int,array<string,mixed>>
 */
function tc_reorder_header_shell_children( array $elements ) {
	$logo = $actions = $nav = null;
	$rest = array();
	...
	foreach ( $elements as $element ) {
		...
		if ( '3e5e84b8' === $id || str_contains( $classes, 'tc-header-logo' ) ) {
			$logo = $element;
		} elseif ( tc_header_element_in_group( $id, 'nav' ) || str_contains( $classes, 'tc-header-nav' ) ) {
			$nav = $element;
		} elseif ( tc_header_element_in_group( $id, 'actions' ) || str_contains( $classes, 'tc-header-actions' ) ) {
			$actions = $element;
		} else {
			$rest[] = $element;
		}
	...
	$ordered = array_values( array_filter( array( $logo, $nav, $actions ) ) );
	if ( count( $ordered ) < 2 ) {
		return $elements;
	}
...
/**
 * CSS de header móvil (contenedor .tc-header-mobile-right agrupado vía JS).
 */
function tc_header_mobile_css() {
	...
	$shell_ids = implode( ',', array_map(
		static fn( $id ) => ".elementor-element-{$id}",
		$layout['shell']
	) );
	$nav_ids = implode( ',', array_map(
		static fn( $id ) => ".elementor-element-{$id}",
		$layout['nav']
	) );
	$action_ids = implode( ',', array_map(
		static fn( $id ) => ".elementor-element-{$id}",
		$layout['actions']
	) );
...
{$scope} :is({$shell_ids})>.e-con-inner{
...
{$scope} :is({$shell_ids})>.e-con-inner>.tc-header-mobile-right{
display:flex!important;
flex-direction:row!important;
flex-wrap:nowrap!important;
align-items:center!important;
...
{$scope} :is({$shell_ids})>.e-con-inner>:is(.elementor-element-3e5e84b8,.tc-header-logo){
...
{$scope} :is({$shell_ids})>.e-con-inner>:is(.elementor-element-3e5e84b8,.tc-header-logo) img{
...
{$scope} .tc-header-mobile-right>:is({$action_ids}){
display:flex!important;
flex-direction:row!important;
align-items:center!important;
gap:4px!important;
...
{$scope} :is({$action_ids}) .hfe-subtotal,
{$scope} :is({$action_ids}) .elementor-button-text.hfe-subtotal{
display:none!important;
}
{$scope} :is({$nav_ids}) .hfe-layout-horizontal .hfe-nav-menu__toggle,
...
{$scope} :is({$nav_ids}) .hfe-nav-menu__toggle:not(.hfe-active-menu)+.hfe-nav-menu__layout-horizontal{
display:none!important;
...
{$scope} :is({$nav_ids}) .hfe-nav-menu__toggle.hfe-active-menu+.hfe-nav-menu__layout-horizontal>ul.hfe-nav-menu{
display:block!important;
visibility:visible!important;
opacity:1!important;
...
{$scope} :is({$nav_ids}) .hfe-nav-menu__toggle.hfe-active-menu+.hfe-nav-menu__layout-horizontal>ul.hfe-nav-menu>li{
width:100%!important;
margin:0!important;
padding:0!important;
}
{$scope} :is({$nav_ids}) .hfe-nav-menu__toggle.hfe-active-menu+.hfe-nav-menu__layout-horizontal>ul.hfe-nav-menu>li>a.hfe-menu-item{
display:block!important;
...
{$scope} :is({$nav_ids}),{$scope} :is({$action_ids}),{$scope} .tc-header-nav,{$scope} .tc-header-actions{
width:auto!important;
...
/**
 * Agrupa iconos + hamburguesa en un contenedor a la derecha (garantiza layout móvil).
 */
function tc_header_mobile_js() {
	$header_id = function_exists( 'get_hfe_header_id' ) ? (int) get_hfe_header_id() : 0;
	if ( $header_id <= 0 ) {
		return '';
	}

	$shell = tc_header_layout_ids()['shell'];
	$shell_js = implode( ',', array_map(
		static fn( $id ) => '.elementor-element-' . $id,
		$shell
	) );
	...
	return <<<JS
(function(){
function tcDecodeIconHtml(raw){
	if(!raw){return '';}
	var tmp=document.createElement('div');
	tmp.innerHTML=raw;
	return tmp.innerHTML;
}
function tcClearMenuStyles(menu){
	if(!menu){return;}
...
function tcPositionMenuDropdown(toggle,menu){
	...
	menu.style.maxWidth='min(300px, calc(100vw - 28px))';
	menu.style.zIndex='100001';
}
function tcCloseMobileMenu(nav){
	var toggle=nav.querySelector('.hfe-nav-menu__toggle');
	var menu=nav.querySelector('nav.hfe-nav-menu__layout-horizontal');
	var icon=nav.querySelector('.hfe-nav-menu-icon');
	if(!toggle||!menu){return;}
	toggle.classList.remove('hfe-active-menu','hfe-active-menu-full-width');
	menu.classList.remove('menu-is-active');
	toggle.setAttribute('aria-expanded','false');
	tcClearMenuStyles(menu);
	if(icon&&menu.getAttribute('data-toggle-icon')){
		icon.innerHTML=tcDecodeIconHtml(menu.getAttribute('data-toggle-icon'));
	}
}
function tcOpenMobileMenu(nav){
	var toggle=nav.querySelector('.hfe-nav-menu__toggle');
	var menu=nav.querySelector('nav.hfe-nav-menu__layout-horizontal');
	var icon=nav.querySelector('.hfe-nav-menu-icon');
	if(!toggle||!menu){return;}
	...
	tcPositionMenuDropdown(toggle,menu);
	if(icon&&menu.getAttribute('data-close-icon')){
		icon.innerHTML=tcDecodeIconHtml(menu.getAttribute('data-close-icon'));
	}
}
function tcInitMobileMenu(nav){
	var toggle=nav.querySelector('.hfe-nav-menu__toggle');
	var menu=nav.querySelector('nav.hfe-nav-menu__layout-horizontal');
	if(!toggle||!menu||toggle.dataset.tcMenuBound){return;}
	toggle.dataset.tcMenuBound='1';
	menu.setAttribute('data-full-width','');
	toggle.addEventListener('click',function(e){
		if(window.innerWidth>1024){return;}
		e.preventDefault();
		e.stopImmediatePropagation();
		if(toggle.classList.contains('hfe-active-menu')){
			tcCloseMobileMenu(nav);
		}else{
			tcOpenMobileMenu(nav);
		}
	},true);
...
function tcUngroupHeaderDesktop(root,logo,actions,nav){
	if(!root.dataset.tcHeaderGrouped){return;}
	var group=root.querySelector('.tc-header-mobile-right');
	if(group){
		if(actions&&actions.parentNode===group){root.appendChild(actions);}
		if(nav&&nav.parentNode===group){root.appendChild(nav);}
		group.remove();
	}
	delete root.dataset.tcHeaderGrouped;
	if(logo&&logo.parentNode===root){root.insertBefore(logo,root.firstChild);}
	if(nav&&actions&&nav.parentNode===root&&actions.parentNode===root){
		root.insertBefore(nav,actions);
	}
}
function tcFixHeaderMobile(){
	var root=document.querySelector('.elementor-{$header_id} :is({$shell_js})>.e-con-inner');
	if(!root){return;}
	var logo=root.querySelector('.elementor-element-3e5e84b8,.tc-header-logo');
	var actions=root.querySelector('.elementor-element-1a7aa264,.elementor-element-128e2831');
	var nav=root.querySelector('.elementor-element-41cdefe5,.elementor-element-2dbc6304');
	if(!logo||!actions||!nav){return;}
	if(window.innerWidth>1024){
		tcCloseMobileMenu(nav);
		tcUngroupHeaderDesktop(root,logo,actions,nav);
		return;
	}
	if(!root.dataset.tcHeaderGrouped){
		var group=root.querySelector('.tc-header-mobile-right');
		if(!group){
			group=document.createElement('div');
			group.className='tc-header-mobile-right';
			root.appendChild(group);
		}
		if(logo.parentNode===root){root.insertBefore(logo,root.firstChild);}
		if(actions.parentNode!==group){group.appendChild(actions);}
		if(nav.parentNode!==group){group.appendChild(nav);}
		root.dataset.tcHeaderGrouped='1';
	}
	var toggle=nav.querySelector('.hfe-nav-menu__toggle');
	if(toggle){
	...
	if(toggle&&toggle.classList.contains('hfe-active-menu')){
		var menu=nav.querySelector('nav.hfe-nav-menu__layout-horizontal');
		if(menu){tcPositionMenuDropdown(toggle,menu);}
	}
}
...
document.addEventListener('DOMContentLoaded',tcFixHeaderMobile);
window.addEventListener('load',tcFixHeaderMobile);
window.addEventListener('resize',tcFixHeaderMobile);
})();
JS;
}

function tc_enqueue_header_mobile_js() {
	if ( is_admin() ) {
		return;
	}

	wp_register_script( 'tc-header-mobile-js', false, array(), TC_SETUP_VERSION, true );
	wp_enqueue_script( 'tc-header-mobile-js' );
	wp_add_inline_script( 'tc-header-mobile-js', tc_header_mobile_js() );
}
...
.elementor-element.tc-header-shell>.e-con-inner{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;align-items:center!important;gap:12px 20px!important;width:100%!important}
.elementor-element.tc-header-shell>.e-con-inner>.elementor-element-3e5e84b8,.elementor-element.tc-header-shell>.e-con-inner>.elementor-element.tc-header-logo,.elementor-element.tc-header-shell>.e-con-inner>.tc-header-logo{flex:0 1 auto!important;min-width:0!important;order:1!important}
...
@media(max-width:1024px){
.elementor-widget-image.tc-header-logo,.elementor-element.tc-header-logo{display:block!important;flex:none!important;width:auto!important;max-width:58%!important;text-align:left!important}
.elementor-widget-image.tc-header-logo img,.tc-header-logo img{max-width:150px!important;max-height:42px!important;margin:0!important}
.elementor-element.tc-header-nav .hfe-nav-menu__layout-horizontal .hfe-nav-menu{display:none!important}
.elementor-element.tc-header-nav .hfe-nav-menu__toggle.hfe-active-menu+.hfe-nav-menu__layout-horizontal>ul.hfe-nav-menu{display:block!important;visibility:visible!important;opacity:1!important}
.elementor-element.tc-header-nav .hfe-nav-menu__toggle.hfe-active-menu+.hfe-nav-menu__layout-horizontal>ul.hfe-nav-menu>li>a.hfe-menu-item{padding:12px 20px!important;text-align:left!important}
...
function tc_apply_header_logo_element( $element, $context ) {
	...
	$element['settings']['align_tablet'] = 'left';
	$element['settings']['align_mobile'] = 'left';
...
function tc_apply_header_menu_element( $element, $context ) {
	...
	$element['settings']['full_width_dropdown'] = '';
...
function tc_home_removed_element_css( $scope = '' ) {
	$prefix = $scope ? $scope . ' ' : '';
	$parts  = array_map(
		static fn( $id ) => $prefix . '.elementor-element-' . $id,
		tc_home_removed_element_ids()
	);

	return implode( ',', $parts ) . '{display:none!important;visibility:hidden!important;height:0!important;overflow:hidden!important;pointer-events:none!important}';
}
...
function tc_process_elementor_elements( $elements, $context = array() ) {
	...
	foreach ( $elements as &$element ) {
		if ( ! empty( $element['elements'] ) ) {
			...
			if ( ! empty( $context['header_template'] ) && tc_header_element_in_group( $element['id'] ?? '', 'shell' ) ) {
				$element['elements'] = tc_reorder_header_shell_children( $element['elements'] );
			}
		}

		if ( ! empty( $context['header_template'] ) && tc_is_header_logo_slot_element( $element ) ) {
			$element = tc_apply_header_logo_element( $element, $context );
			continue;
		}
...
function tc_home_responsive_css( $home_id ) {
	...
	return $grids . '
' . tc_home_removed_element_css( $scope ) . '
@media(max-width:1024px){
